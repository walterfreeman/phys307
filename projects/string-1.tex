...
\centerline{Due Tuesday, 12 November, at 5 PM}
\centerline{Progress report due 6 November before class}
\normalsize

This homework assignment is the first of several parts. I encourage you to try to finish this assignment ahead of time
...
\begin{align}
  \dot \vec v_{x_i} &= \frac{1}{m} \left[ k \hat r_{{i,i-1}_x} (r_{i,i-1} - r_0) + k \hat r_{i,i+1} (r_{i,i+1} - r_0) \right] \\
  \dot \vec v_{y_i} &= \frac{1}{m} \left[ k \hat r_{{i,i-1}_y} (r_{i,i-1} - r_0) + k \hat r_{i,i+1} (r_{i,i+1} - r_0) \right]
\end{align}
