...
{
  static float X,Y,Z,X2,Y2,Z2;
  transform(x1,y1,z1,&X,&Y,&Z);
  transform(x2,y2,z2,&X2,&Y2,&Z2);
  if (Z<-vdist || Z2<-vdist) return;
//  if (Z<-vdist*2 || Z2<-vdist*2) return;
  if (ctog)  myColor4f(0.5,0.5,0.5,exp(-(Z*4+vdist)/vdist)+.15);
  glVertex3f(X,Y,0);
  if (ctog)  myColor4f(0.5,0.5,0.5,exp(-(Z2*4+vdist)/vdist)+.15);
  glVertex3f(X2,Y2,0);
}
...
//    y = y/(z+vdist)*(vdist/scale);
  ...
  else if (line[0] == 't' && line[1] != '3') // text, not 3d
  {
    ...
    glBegin(GL_LINES);
      glVertex3f((x1+r)/scale, (y1)/scale,0);
    for (i=0;i<num_lines;i++)
    {
      angle = i*2*M_PI/num_lines;
  ...
  else if (line[0] == 'Q') {fflush(stdout); exit(0);}
...
void keyb(unsigned char key, int x, int y)
{
  ...
  if (key == ',') {contrast /= 1.2; update=1;}
  if (key == '.') {contrast *= 1.2; update=1;}
  if (key == '=') {scale /= 1.02; update=1;}
  if (key == '-') {scale *= 1.02; update=1;}
  if (td) {
    if (key == 'a') {theta2 += 0.1; update=1;}
    if (key == 'd') {theta2 -= 0.1; update=1;}
    if (key == 'q') {phi2   += 0.1; update=1;}
    if (key == 'e') {phi2   -= 0.1; update=1;}
    if (key == 'w') {psi2   += 0.1; update=1;}
    if (key == 's') {psi2   -= 0.1; update=1;}
    if (key == '_') {vdist *= 1.02; update=1;}
    if (key == '+') {vdist /= 1.02; update=1;}
  }
  else
  {
    if (key == 'a') {xcenter += 0.02*scale;update=1;}
    if (key == 's') {ycenter += 0.02*scale;update=1;}
    if (key == 'd') {xcenter -= 0.02*scale;update=1;}
    if (key == 'w') {ycenter -= 0.02*scale;update=1;}
  }
