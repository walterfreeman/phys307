...
File names in Linux, like everything else, are case sensitive. It's generally a good idea to use lowercase for everything unless you have a specific reason not to. They can contain spaces, but this is a giant pain in the butt and you shouldn't do it; use dashes or underscores
instead.
...
Try this! Note that it's running a bit slowly: this is because the Firefox window, and everything in it, is actually being run elsewhere on campus, and the graphics are being streamed to you. Years before Twitch was invented, Linux computers were happily streaming graphics
to each other.
...
#### 2.3 `cd` and `pwd`: change directory, and print working directory
...
If you ever want to know what directory you're in, type `pwd`.
...
`kate` and `gedit`: These are more "modern" graphical editors which may run slower over a remote graphics connection from home, but provide an attractive environment for you to code in similar to the things you're used to in Windows.

As with all programs, you can open a specific file by doing e.g. `gedit helloworld.c`.

#### 2.9 `g++`, the C compiler
...
#### 2.10 `python`, the Python interpreter

If you want to run Python code, there is no compiler. Instead just put your code in a file ending in `.py`, and say for instance
`python program.py`. If you want to test out Python commands, you can invoke the REPL by just saying `python`.
...
You may also pipe text to `plot` by doing something like
