\documentclass[12ampt]{article}   % there are other document classes, but "article" works fine
\setlength\parindent{0pt}     %   don't indent paragraphs
\usepackage{fullpage}         %   remove page numbers etc
\usepackage{amsmath}          %   American Mathematical Society math stuff. Not sure what this does
\setlength{\parskip}{4mm}

\begin{document}

% text sizes are tiny, scriptsize, footnotesize, small, normalsize, large, Large, huge, and Huge
\Large
\centerline{\sc{Notes on error analysis of numerical integration}} 
\normalsize

% surrounding things with $ signs switches to math mode. Surrounding them with $$ is shorthand for \begin{equation}, which sets them apart on a new line
...
% align: multiple equations on different lines that have different bits aligned. \\ ends line; & is the alignment character.
\begin{align}
...
% \mathrm = use Roman type, like \rm. _ is for subscripts, ^ for superscripts.
\begin{equation}
  f_{\mathrm{LHR}}(x) = f(0)
\end{equation}
...
% itemize gives you a bulleted list; enumerate gives you a numbered one. These nest properly.
\begin{itemize}
